...
def plot_training_curves(train_losses, test_losses, train_accuracies, test_accuracies, save_path):
    ...
    plt.tight_layout()
    plt.savefig(save_path, dpi=int(300), bbox_inches='tight')
    plt.close()
    print(f"Training curves saved to {save_path}")

def visualize_cifar_samples(dataloader, num_samples=16, save_path=None):
    """
    ...
    ax.imshow(grid_np)
    ax.set_title(f'CIFAR-10 Samples')
    ax.axis('off')
    ...
    for i, label in enumerate(labels):
        ...
        ax.text(col * 8.5 + 4, row * 8.5 + 4, class_name, 
    ...
    if save_path:
        plt.savefig(save_path, dpi=int(300), bbox_inches='tight')
        print(f"CIFAR-10 samples visualization saved to {save_path}")
...
def visualize_predictions(model, dataloader, device, num_samples=16, save_path=None):
    ...
    ax.imshow(grid_np)
    ax.set_title('Model Predictions on CIFAR-10 Test Samples')
    ax.axis('off')
    ...
    for i, (true_label, pred_label) in enumerate(zip(true_labels, predicted_labels)):
        row = i // 4
        col = i % 4
        true_class = CIFAR10_CLASSES[true_label.item()]
        pred_class = CIFAR10_CLASSES[pred_label.item()]
        color = 'green' if true_label == pred_label else 'red'
        
        # True label
        ax.text(col * 8.5 + 4, row * 8.5 + 2, f'True: {true_class}', 
                ha='center', va='center', color='blue', fontsize=9,
                bbox=dict(boxstyle="round,pad=0.2", facecolor='white', alpha=0.8))
        
        # Predicted label
        ax.text(col * 8.5 + 4, row * 8.5 + 6, f'Pred: {pred_class}', 
